...
async function deleteCommand(options) {
    try {
        ...
        if (!options.local && !options.cloud && !options.both) {
            ...
            console.log(styles.help(`Use ${styles.value('-l')} or ${styles.value('--local')} to delete local entries`));
            console.log(styles.help(`Use ${styles.value('-c')} or ${styles.value('--cloud')} to delete cloud entries`));
            console.log(styles.help(`Use ${styles.value('-b')} or ${styles.value('--both')} to delete all entries`));
            return;
        }
        ...
        const { confirm } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'confirm',
                message: styles.prompt(`Are you sure you want to delete your entries from ${styles.value(storageToDelete)} storage?`),
                default: false
            }
        ]);

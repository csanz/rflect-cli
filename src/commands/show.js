...
async function showCommand(options) {
  try {
    ...
    if (!options.date && !options.all && !options.recent) {
      ...
      console.log(
        styles.help(
          `Use ${styles.value('-r')} or ${styles.value('--recent')} to see your most recent entry saved in ${styles.value(storagePreference)} storage.`
        )
      );
      console.log(
        styles.help(
    ...
    if (error.code === 'ENOENT') {
      ...
      console.log(
        styles.error(
          `Error occurred when trying to receive your past entries: ${styles.value(error.message)}`
        )
      );
    }
  }

...
async function storageCommand(options) {
  try {
    ...
    if (!options.local && !options.cloud && !options.both) {
      ...
      console.log(
        styles.help(
          `Use ${styles.value('-l')} or ${styles.value('--local')} for filesystem storage only`
        )
      );
      console.log(
        styles.help(
          `Use ${styles.value('-c')} or ${styles.value('--cloud')} for cloud storage only`
        )
      );
      console.log(
        styles.help(
          `Use ${styles.value('-b')} or ${styles.value('--both')} for both local and cloud storage`
        )
      );
      return;
    }
    ...
    if (
      ...
      (options.local || options.cloud || options.both)
    ) {
      ...
      const { confirm } = await inquirer.prompt([
        {
          ...
          message: styles.prompt(
            'Would you like to migrate your existing entries?'
          ),
          default: true,
        },
      ]);
      ...
      if (!confirm) {
        console.log(
          styles.warning('Migration cancelled. Storage preference not updated.')
        );
        return;
      }
    ...
    console.log(
      styles.info(
        `Your entries will now be saved to ${styles.value(newPreference)} storage.`
      )
    );
    ...
    console.log(
      styles.error('\nError updating storage settings: ') +
